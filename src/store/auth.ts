...
interface AuthState {
  status: AuthStatus;
  step: AuthStep;
  /** Richtung des letzten Schrittwechsels — steuert die Schiebe-Animation. */
  direction: 1 | -1;
  email: string;
  rememberMe: boolean;
  /** Sekunden, bis ein neuer Code angefordert werden darf. */
  resendIn: number;
  busy: boolean;
  user: AuthUser | null;
...
export const useAuthStore = create<AuthState>()((set, get) => {
  ...
  function completeLogin(email: string) {
    ...
    set({ status: "signed-in", user, error: null, busy: false });
  }
